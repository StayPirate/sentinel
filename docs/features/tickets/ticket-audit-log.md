...
| `track_status_changed` | Track status changed (VA action, admin force-FIXED, or release detection) | Acting user for human changes, `NULL` for automatic transitions (e.g., release detected sets FIXED) | Old status | New status | `NULL` | `{"track": "...", "package": "..."}` (see detail contract) |
