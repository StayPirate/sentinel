...
|---------|------|---------|-------------|------------|
| `default_cvss_version` | string | `"3.1"` | Preferred version in the cross-version Severity Resolution Cascade and exact canonical SUSE version for Eligibility Score Resolution. Other accepted CVSS versions are not excluded from severity resolution. Allowed: `"3.1"`, `"4.0"` | `docs/features/platform/system-settings.md` |
